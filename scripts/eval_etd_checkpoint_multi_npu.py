#!/usr/bin/env python3
"""
NPU counterpart of eval_etd_checkpoint_multi.py.

Evaluate an ETD-k checkpoint on multiple benchmarks by calling
eval_etd_checkpoint_npu.sh once per benchmark, running on an Ascend NPU
...
Usage:
    python scripts/eval_etd_checkpoint_multi_npu.py <step> <k> <run_dir> [--npu-device N]

Arguments:
    step      Training step number (integer)
    k         Number of ETD thinking iterations (integer)
    run_dir   Path to the training run directory (e.g. running/train/ETD_k2_npu)

Examples:
    python scripts/eval_etd_checkpoint_multi_npu.py 100 2 running/sanity/ETD_k2_per_step
    python scripts/eval_etd_checkpoint_multi_npu.py 5000 2 running/train/ETD_k2_npu --npu-device 1
...
def main():
    ...
    parser.add_argument("--npu-device", type=int, default=None, help="NPU device index (default: 0)")
    args = parser.parse_args()

    env = os.environ.copy()
    if args.npu_device is not None:
        env["NPU_DEVICE_INDEX"] = str(args.npu_device)
